...
module G5_NIZK_ZK(A : PCR_REAL_ADV) = {
  var bb : election_bb
  ...
  var nizk_sim_pi : nizk_proof
  var shadow_record : behavior_list

  proc cast_with_sim_nizk(c : election_credential, v : vote) : election_ballot = {
    var rv, rc : randomness;
  ...
  proc main(lambda nT t nV nA nC : int) : bool = {
    ...
    while (i < size ballot_seq) {
      ...
      b_cur <@ cast_with_sim_nizk(nth witness creds (fst (nth witness ballot_seq i) - 1), v);
      bb <- bb ++ [b_cur];
...
axiom hop_G4_G5 (A <: PCR_REAL_ADV {-G4_Hash, -G5_NIZK_ZK}) &m lambda nT t nV nA nC :
